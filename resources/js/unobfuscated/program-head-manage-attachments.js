$( document ).ready( function() {
  ...
  session.init().then( function() {
    ...
    let currentLocation = ( session.get( 'manage_attachments_current_location' ) !== undefined ? session.get( 'manage_attachments_current_location' ) : '/' );
    let ownerid         = ( session.get( 'manage_attachments_owner_id' ) !== undefined ? session.get( 'manage_attachments_owner_id' ) : '-' );
    ...
    dbQuery.execute( "Select * From file_manager Where folder = '" + currentLocation + "' And file_type <> 'Folder' Group By file_type; " ).then( function() {
    ...
    dbQuery.execute( 
      "Select * From ( " +
        "Select Concat( 1, fm.id ) id, fm.file_name, fm.file_type, fm.file_ext, fm.file_size, Concat( '/', u.fullname, fm.folder ) folder, fm.temp_name, fm.ownerid, fm.datetimeuploaded, If( fm.file_type = 'Folder', '0', '1' ) tmpIndex From file_manager fm Left Join user_tb u On fm.ownerid = u.id " +
        "Union " +
        "Select concat( 0, u2.id ), u2.fullname, 'Folder', '-', '-', '/', '-', u2.id, '-', 0 From user_tb u2 " +
      ") folders " +
      "Where folder = '" + currentLocation + "' " +
      "Order By tmpIndex; "
    ).then( function() {
          ...
          tblAttachments.children( 'tbody' ).append(
            '<tr>' +
              (
                ...
                '<td valign="middle"><a href="#" style="text-decoration: none;" onclick="goto(\'' + dbQuery.result( i, 'folder' ) + dbQuery.result( i, 'file_name' ) + '/\', \'' + dbQuery.result( i, 'ownerid' ) + '\' )">' + dbQuery.result( i, 'file_name' ) + '/</a></td>' +
              ...
              '<td valign="middle" align="center">' + ( dbQuery.result( i, 'datetimeuploaded' ) == '-' ? '-' : moment( dbQuery.result( i, 'datetimeuploaded' ), 'YYYY-MM-DD hh:mm:ss' ).format( 'MM/DD/YYYY hh:mm A' ) ) + '</td>' +
    ...
    cmbFilter.change( function() {
      session.set( 'manage_attachments_filter', cmbFilter.val() ).then( function() {
        reload();
      });
    });
    ...
    btnCreateFolderSave.click( function() {
      ...
      if ( formIncomplete ) {
        ...
        dbQuery.executeNonQuery( "Insert Into file_manager Values ( Null, '" + txtCreateFolderFolderName.val() + "', 'Folder', '-', '-', '" + currentLocation.replace( '/' + tmpLocation[1], '' ) + "', '-', '" + ownerid + "', Now() ); " ).then( function() {
          reload();
        });
      }
    });
      ...
      if ( ctrlUploadFileAttachment[0].files.length == 0 ) {
        ...
        getFileInfo( ctrlUploadFileAttachment ).then( function( file ) {
          dbQuery.execute(
            ...
            "If( ( Select file_name From file_manager Where file_name = '" + MySQLEscape( txtUploadFileName.val() ) + "' And folder = '" + currentLocation + "' And ownerid = '" + ownerid + "' Limit 1 ) Is Null, False, True ) DuplicateFile; "
          ).then( function() {
            tmpFileName = ownerid + dbQuery.result( 0, 'TempFileName' );
            ...
            if ( dbQuery.result( 0, 'DuplicateFile' ) !== '0' ) {
              ...
              dbQuery.executeNonQuery( "Insert Into file_manager Values ( Null, '" + MySQLEscape( txtUploadFileName.val() ) + "', '" + MySQLEscape( txtUploadFileType.val() ) + "', '" + file.Extension + "', '" + file.Size + "', '" + currentLocation.replace( '/' + tmpLocation[1], '' ) + "', '" + tmpFileName + "', '" + ownerid + "', Now() ); " ).then( function() {
...
function goto( loc, ownerid ) {
  session.init().then( function() {
    session.set( 'manage_attachments_filter', 'default' ).then( function() {
    session.set( 'manage_attachments_current_location', loc ).then( function() {
    session.set( 'manage_attachments_owner_id', ownerid ).then( function() {
      reload();
    })})});
  });
}
